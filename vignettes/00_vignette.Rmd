...
```
In addiction to the High, Low, Open, Close, and Volume variables we have added 3 more variables that are: 

- **Adj**: the adjusted price is computed as the mean price between the High, Low, Open and Close.
...
Changing the interval from *daily* to *hourly* allow to retrieve the hourly candles for the pair specified. 
...
- the **api key** is not requested for data up to 7 days, for more data the key is needed.
